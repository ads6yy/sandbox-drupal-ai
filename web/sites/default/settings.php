 ...
 * @endcode
 */
$settings['hash_salt'] = getenv('HASH_SALT') ?: 'drupal-ai-sandbox-local-dev-salt';
...
# }
/**
 * Connexion base de données par variables d'environnement (cf. deploy/.env).
 * Aucun secret n'est versionné : les valeurs viennent du conteneur.
 */
$databases['default']['default'] = [
  'driver' => 'mysql',
  'host' => getenv('DB_HOST') ?: 'mariadb',
  'port' => getenv('DB_PORT') ?: '3306',
  'database' => getenv('DB_NAME') ?: 'drupal-ai',
  'username' => getenv('DB_USER') ?: 'drupal-ai',
...
/**
 * Utiliser les prompts stockés dans les entités AI Assistant (sinon le module
 * recharge ses fichiers resources/*.txt et ignore toute personnalisation).
 */
$settings['ai_assistant_custom_prompts'] = TRUE;

/**
 * Langfuse : connexion par variables d'environnement (jamais en config exportée).
 */
if (getenv('LANGFUSE_PUBLIC_KEY')) {
  $config['langfuse.settings']['langfuse_url'] = getenv('LANGFUSE_BASE_URL') ?: 'https://cloud.langfuse.com';
  $config['langfuse.settings']['auth_method'] = 'key_pair';
  $config['langfuse.settings']['key_pair']['public_key'] = getenv('LANGFUSE_PUBLIC_KEY');
  $config['langfuse.settings']['key_pair']['secret_key'] = getenv('LANGFUSE_SECRET_KEY');
}
